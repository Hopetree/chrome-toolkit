...
- **交互形态**：纯 Popup 模式（520px 宽，最小 200px 高）
...
```
宽度：  520px（固定）
高度：  自适应内容，最小 200px（无最大高度限制，由浏览器 Popup 自身管理）
内边距：16px
```

**滚动条**：由浏览器 Popup 原生管理，不自定义 overflow 属性。通过 `::-webkit-scrollbar` 统一滚动条样式：

```
宽度：     6px
轨道：     透明
滑块：     --border 颜色（#e5e5e5），3px 圆角
滑块悬停： --text-muted 颜色（#737373）
```
